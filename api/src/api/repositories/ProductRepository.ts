...
export class ProductRepository extends Repository<Product> {
    ...
    public async productList_bycategory(select: any = [], searchConditions: any = [], whereConditions: any = [], categoryId: any = [], count: number | boolean,vendor:number): Promise<any> {
        ...
        //     query.orderBy('product.price', price === 1 ? 'ASC' : 'DESC');
        // }
        query.andWhere('(product.vendor = :vendor )', {vendor});
        query.orderBy('product.sortOrder', 'ASC');
    ...
    public async productListLatest(limit: number, offset: number,   count: number | boolean): Promise<any> {
        ...
        sql += 'from product p where p.created_date >= DATE_ADD(CURDATE(), INTERVAL - 3 DAY) '; // last 3 days
    ...
    public async adsProductList(limit: number, offset: number, code: string): Promise<any> {        
        const query: any = await this.manager.createQueryBuilder(Product, 'product');
        // Select
        query.where('product.proCode = "' +code+'"');
    ...
    public async recentProductSelling(limit: number): Promise<any> {
        ...
        const query = await this.manager.createQueryBuilder(OrderProduct, 'orderProduct');
        query.select(['COUNT(orderProduct.order_id) as ordercount', 
        'orderProduct.product_id as productId']);
        query.groupBy('productId');
        query.innerJoin('product','products','orderProduct.product_id = products.product_id');
        query.where('products.is_active = :active',{active:1});
        query.where('products.forsell = :forsell',{forsell:1});
        query.where('products.online = :online',{online:1});
    ...
    public async customProductList(limit: number, offset: number, categoryId: any = [], manufacturerId: number, condition: number, keyword: string, priceFrom: string, priceTo: string, price: string): Promise<Product[]> {
        ...
        let sql: any = 'SELECT p.product_id as productId, p.price, p.sku, p.upc, p.quantity,p.forsell,p.online, p.stock_status_id as stockStatusId, p.manufacturer_id as manufacturerId, p.date_available as dateAvailable,' +
            ' p.sort_order as sortOrder, p.name, p.description, p.amount, p.condition, p.meta_tag_title as metaTagTitle, p.meta_tag_description as metaTagDescription, p.meta_tag_keyword as metaTagKeyword , p.is_active as is_active'  + ' ';
    ...
    public async categoryProductList(limit: number, offset: number, categoryId: any = [], manufacturerId: number, condition: number, keyword: string, priceFrom: string, priceTo: string, price: string,vendor:number): Promise<Product[]> {
        ...
        let sql: any = 'SELECT p.product_id as productId, p.price, p.sku, p.upc, p.quantity,p.vendor as vendorId, p.stock_status_id as stockStatusId, p.manufacturer_id as manufacturerId, p.date_available as dateAvailable,' +
            ' p.sort_order as sortOrder, p.name, p.description, p.amount, p.condition, p.meta_tag_title as metaTagTitle, p.meta_tag_description as metaTagDescription, p.meta_tag_keyword as metaTagKeyword , p.is_active as is_active'  + ' ';
