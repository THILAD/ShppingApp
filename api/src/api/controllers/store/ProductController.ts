...
export class ProductController {
    ...
    @Get('/top-selling-productlist-noAuthorized')
    // @Authorized()
    public async topSellingProductList(@Req() request: any, @Res() response: any): Promise<any> {
        ...
        const data = await this.productService.recentProductSelling(5);
     ...
     */
    // Order Detail Function
    @Get('/top-selling-productlist-noAuthorized-forvendor')
    // @Authorized()
    public async topSellingProductListForvendor(@QueryParam('vendorid') vendorid:number, @Req() request: any, @Res() response: any): Promise<any> {
        //const user = await this.authService.parseBasicAuthFromRequest(request);

        const data = await this.productService.recentProductSelling_vendor(5,vendorid);
        const promise = data.map(async (result: any) => {
            const product = await this.productService.findOne({
                select: ['productId', 'image', 'imagePath', 'price', 'name', 'description','vendor'],
                where: [
            ...
            console.log('vvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvv',vendorid);
