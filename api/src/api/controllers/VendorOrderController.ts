...
export class VendorOrderController {
    ...
    public async orderList(@QueryParam('limit') limit: number, @QueryParam('offset') offset: number, @QueryParam('orderId')orderId: string, @QueryParam('orderStatusId')orderStatusId: string, @QueryParam('customerName')customerName: string,
                           @QueryParam('totalAmount')totalAmount: string, @QueryParam('keyword')keyword: string, @QueryParam('dateAdded')dateAdded: string, @QueryParam('count')count: number | boolean, @Req() request: any, @Res() response: any): Promise<any> {
        const select = ['orderId', 'orderStatusId', 'orderPrefixId', 'shippingFirstname', 'total', 'createdDate', 'customerId', 'isActive', 'modifiedDate','vendor','clear'];
        const search = [
            {
                name: 'orderPrefixId',
                op: 'like',
        ...
        const orderList = await this.orderService.list_vendor(limit, offset, select, search,search_or, WhereConditions, 0, count,user.id);
        if (count) {
            const Response: any = {
                status: 1,
    ...
    @Get('/orderlist_only')
    @Authorized()
    public async orderList_only(@QueryParam('limit') limit: number, @QueryParam('offset') offset: number, @QueryParam('orderId')orderId: string, @QueryParam('orderStatusId')orderStatusId: string, @QueryParam('customerName')customerName: string,
                           @QueryParam('totalAmount')totalAmount: string, @QueryParam('keyword')keyword: string, @QueryParam('dateAdded')dateAdded: string, @QueryParam('count')count: number | boolean, @Req() request: any, @Res() response: any): Promise<any> {
        const select = ['orderId', 'orderStatusId', 'orderPrefixId', 'shippingFirstname', 'total', 'createdDate', 'customerId', 'isActive', 'modifiedDate','vendor','clear'];
        const search = [
            {
        ...
        // user.id user.usertype
        const WhereConditions = [];
        console.log('----------------------------   list_vendor_only       ---------------------------------------------------');
        
        const orderList = await this.orderService.list_vendor_only(limit, offset, select, search,search_or, WhereConditions, 0, count,user.id);
        if (count) {
            const Response: any = {
                status: 1,
                message: 'Successfully got count.',
    ...
    @Get('/stock_orderlist')
    @Authorized()
    public async stock_orderList(@QueryParam('limit') limit: number, @QueryParam('offset') offset: number, @QueryParam('orderId')orderId: string, @QueryParam('orderStatusId')orderStatusId: string, @QueryParam('customerName')customerName: string,
                           @QueryParam('totalAmount')totalAmount: string, @QueryParam('keyword')keyword: string, @QueryParam('dateAdded')dateAdded: string, @QueryParam('count')count: number | boolean, @Req() request: any, @Res() response: any): Promise<any> {
        const select = ['orderId', 'orderStatusId', 'orderPrefixId', 'shippingFirstname', 'total', 'createdDate', 'customerId', 'isActive', 'modifiedDate','vendor'];
        const search = [
            {
                name: 'orderPrefixId',
                op: 'like',
    ...
    public async orderDetail(@QueryParam('orderId') orderid: number, @Req() request: any, @Res() response: any): Promise<any> {
        const orderData = await this.orderService.findOrder({where: {orderId: orderid}, select: ['orderId', 'orderStatusId', 'customerId', 'telephone', 'invoiceNo', 'invoicePrefix', 'orderPrefixId', 'shippingFirstname', 'shippingLastname', 'shippingCompany', 'shippingAddress1',
            ...
            'paymentPostcode', 'paymentCountry', 'paymentZone', 'paymentAddressFormat', 'total','discount', 'customerId', 'createdDate']});
        orderData.productList = await this.orderProductService.find({where: {orderId: orderid}, select: ['orderProductId', 'orderId', 'productId', 'name', 'model', 'quantity', 'total', 'productPrice','status']});
    ...
    @Get('/saleslist')
    @Authorized()
    public async salesList(@Res() response: any , @Req() request: any): Promise<any> {
        const user = await this.authService.parseBasicAuthFromRequest(request);
        const orderList = await this.orderService.salesList_vendor(user.id);
    ...
    @Authorized()
    public async totalOrderAmount(@Res() response: any, @Req() request: any): Promise<any> {
        let total = 0;
        const user = await this.authService.parseBasicAuthFromRequest(request);
        const order = await this.orderService.findAll_vendor(user.id);
        let n = 0;
    ...
    @Get('/total-order-amount-clear')
    @Authorized()
    public async totalOrderAmount_clear(@Res() response: any, @Req() request: any): Promise<any> {
        let total = 0;
        const user = await this.authService.parseBasicAuthFromRequest(request);
        const order = await this.orderService.findAll_vendor_clear(user.id);
        let n = 0;
        for (n; n < order.length; n++) {
            total += +order[n].total;
        }
    ...
    @Authorized()
    public async todayOrderAmount(@Res() response: any, @Req() request: any): Promise<any> {
        const user = await this.authService.parseBasicAuthFromRequest(request);
        const nowDate = new Date();
        const todaydate = nowDate.getFullYear() + '-' + (nowDate.getMonth() + 1) + '-' + nowDate.getDate();
        console.log(todaydate);
        let total = 0;
        const order = await this.orderService.findAlltodayOrder_vendor(todaydate,user.id);
        let n = 0;
        for (n; n < order.length; n++) {
            total += +order[n].total;
        }
    ...
    @Authorized()
    public async orderCount(@Res() response: any, @Req() request: any): Promise<any> {
        const user = await this.authService.parseBasicAuthFromRequest(request);
        const nowDate = new Date();
        const todaydate = nowDate.getFullYear() + '-' + (nowDate.getMonth() + 1) + '-' + nowDate.getDate();

        const orderCount = await this.orderService.findAllTodayOrderCount_vendor(todaydate,user.id);
        const successResponse: any = {
            status: 1,
            message: 'Successfully get Today order count',
            data: orderCount,
    ...
    @Post('/order-clear-debit')
    @Authorized()
    public async orderClear(@Body({validate: true}) orderChangeStatus: UpdateOrderChangeStatus, @Res() response: any): Promise<any> {
        // return new Promise(async () => {
        const updateOrder = await this.orderService.findOrder(orderChangeStatus.orderId);
        console.log(updateOrder);
        if (!updateOrder) {
        ...
        updateOrder.clear = orderChangeStatus.orderStatusId; // clear = 0 or 1
        console.log(updateOrder.clear );
    ...
    @Get('/ticket_report')
    @Authorized()
    public async ticket_report(@QueryParam('date_start') date_start: string,@QueryParam('date_end')date_end: string, @Req() request: any, @Res() response: any): Promise<any> {
        const select = ['orderId', 'orderStatusId', 'orderPrefixId', 'shippingFirstname', 'total', 'createdDate', 'customerId', 'isActive', 'modifiedDate','vendor','clear'];
        ...
        const orderList = await this.orderService.list_vendor_report( select, date_start, date_end,user.id);
        console.log('-------------------------------------------------------------------------------');
        const orderStatus = orderList.map(async (value: any) => {
            const status = await this.orderStatusService.findOne({
                where: {orderStatusId: value.order_status_id},
                select: ['orderStatusId', 'name', 'colorCode'],
            });
            const temp: any = value;
            temp.orderStatus = status;
        ...
        const successResponse: any = {
            status: 1,
            message: 'Successfully report ticket list.',
            data: results,
        };
        return response.status(200).send(successResponse);
    }
    ...
    @Get('/order_report')
    @Authorized()
    public async order_report(@QueryParam('date_start') date_start: string,@QueryParam('date_end')date_end: string, @Req() request: any, @Res() response: any): Promise<any> {
        const user = await this.authService.parseBasicAuthFromRequest(request);
        const productList = await this.orderProductService.report_order(user.id,date_start,date_end,'0,1');
        // const orderStatusData = await this.orderStatusService.findOne({
        //         where: {orderStatusId: orderData.orderStatusId},
        //         select: ['name', 'colorCode']});
        // orderData.orderStatusName = orderStatusData.name;
        ...
        const successResponse: any = {
            status: 1,
            message: 'Successfully shown the order report. ',
            data:  productList,
        };
        return response.status(200).send(successResponse);
    }

    @Get('/import_stock_report')
    @Authorized()
    public async import_stock_report(@QueryParam('date_start') date_start: string,@QueryParam('date_end')date_end: string, @Req() request: any, @Res() response: any): Promise<any> {
        const user = await this.authService.parseBasicAuthFromRequest(request);
        const productList = await this.orderProductService.report_order(user.id,date_start,date_end,'2');
        // const orderStatusData = await this.orderStatusService.findOne({
        //         where: {orderStatusId: orderData.orderStatusId},
        //         select: ['name', 'colorCode']});
        // orderData.orderStatusName = orderStatusData.name;
    ...
    @Get('/export_stock_report')
    @Authorized()
    public async export_stock_report(@QueryParam('date_start') date_start: string,@QueryParam('date_end')date_end: string, @Req() request: any, @Res() response: any): Promise<any> {
        const user = await this.authService.parseBasicAuthFromRequest(request);
        const productList = await this.orderProductService.report_order(user.id,date_start,date_end,'3');
        // const orderStatusData = await this.orderStatusService.findOne({
        //         where: {orderStatusId: orderData.orderStatusId},
        //         select: ['name', 'colorCode']});
        // orderData.orderStatusName = orderStatusData.name;
