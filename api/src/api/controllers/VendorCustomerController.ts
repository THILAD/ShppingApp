...
export class VendorCustomerController {
    ...
    public async customerList(@QueryParam('limit') limit: number, @QueryParam('offset') offset: number, @QueryParam('name') name: string, @QueryParam('status') status: string, @QueryParam('email') email: string, @QueryParam('customerGroup') customerGroup: string, @QueryParam('date') date: string, @QueryParam('count')count: number | boolean,@Req() request: any, @Res() response: any): Promise<any> {
        ...
        const customerList = await this.customerService.list_vendor(limit, offset, search, WhereConditions, 0, count,user.id);
...
// Get Customer Detail API
    ...
    public async recentCustomerList(@Req() request: any,@Res() response: any): Promise<any> {
        ...
        const user = await this.authService.parseBasicAuthFromRequest(request);
        const customerList = await this.customerService.list_vendor(0, 0, 0, WhereConditions, order, 0,user.id);
        const successResponse: any = {
            status: 1,
            message: 'Successfully got Customer list.',
