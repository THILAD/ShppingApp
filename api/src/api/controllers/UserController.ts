...
export class UserController {
    ...
    public async login(@Body({ validate: true }) loginParam: LoginRequest, @Res() response: any): Promise<any> {
        ...
        if (user) {
            if (await User.comparePassword(user, loginParam.password)) {
                // create a token
                const token = jwt.sign({ id: user.userId, usertype: 'admin' }, '123##$$)(***&');
     ...
     */
    @Post('/check-user')
    public async checkUser(@Body({ validate: true }) createParam: CreateRequest, @Res() response: any): Promise<any> {
    ...
    public async forgotPassword(@Body({ validate: true }) forgotPassword: ForgotPassword, @Res() response: any): Promise<any> {
        ...
        const phoneNumber = forgotPassword['phonenumber'];
        const isvalid = AccessTokenService.verifyGoogleIdToken(googleTokenId, phoneNumber, 'laoapp');//laoappssell
        if (!isvalid) {
            const errorResponse: any = {
                status: 0,
                message: 'Invalid google token id',
