...
export class MediaController {
    ...
    @Post('/upload-files')
    // @Authorized()
    public async uploadFiles(@Body({ validate: true }) fileNameRequest: FileNameRequest[], @Req() request: any, @Res() response: any): Promise<any> {
        let successResponse:any={
            status: 1,
            message: 'Image successfully uploaded',
            data: [],
        }
        for (let index = 0; index < fileNameRequest.length; index++) {
            const element =fileNameRequest[index];
            console.log('element ',element);
            const base64 = element.image;
