...
export class ProductController {
    ...
    public async addProduct(@Body({validate: true}) product: AddProductRequest, @Req() request: any, @Res() response: any): Promise<any> {
        ...
        newProduct.proCode = product.proCode;
        console.log('------------------------------------------------------------');
        console.log(newProduct);

   
        const proCode: any = await this.adsService.findOne({
            where: {
        ...
        // if(!proCode.proCode){
        //     console.log('!!!!!!!!!!!!!!!!adsID!!!!!!!!!!!!',proCode.proCode);
        //     const errorResponse: any = {
        //         status: 0,
        //         message: 'sssdsldl mua',
        //     };
        //     return response.status(400).send(errorResponse);
        // }

        if (proCode) {
            const productS: any = await this.productService.find({
                where: {
                    proCode: product.proCode,
                },
            });
            ...
            console.log('**************************procode*****************************',product.proCode);
            if (productS.length >= proCode.allowedPromote) {
                const errorResponse: any = {
                    status: 0,
                    message: 'Maximum Procode'+proCode.allowedPromote,
                };
                return response.status(400).send(errorResponse);
            }
        ...
        if (product.categoryId) {
            ...
            for (const categoryId of category) {
                ...
                console.log(newProductToCategory);
    ...
    @Authorized()
    public async updateProduct(@Body({validate: true}) product: UpdateProductRequest, @Res() response: any,@Req() request:any ): Promise<any> {
        console.log(product);
        ...
        // console.log('vendorrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrr',product.vendor);
        ...
        //     },
        // });
        // console.log('----------------vendor---------------',updateProduct.vendor)
        ...
        //     return response.status(400).send(errorResponse);
        // }
        // console.log('cccccccccccccccoooooooooooooooooooodddddddddddddeeeeeeee',product.proCode);
        ...
        if (!proCode) {
            const errorResponse: any = {
                status: 0,
                message: 'Invalid procode',
            };
            return response.status(400).send(errorResponse);
        }

        const p: any = await this.productService.find({
            where: {
                proCode: product.proCode,
                vendor: updateProduct.vendor,
            },
        });
        console.log(product.proCode+' > '+proCode.allowedPromote);
        console.log('xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx',updateProduct.vendor);
        console.log('ppppppppppppppppppppppppppppppppppp length',p.length);
        
        if(p.length >= proCode.allowedPromote){
            console.log('---------------------allowPromote-----------------',proCode.allowedPromote);
            console.log('run :'+product.proCode+' >= '+proCode.allowedPromote);
            const errorResponse: any = {
                status: 0,
                message: 'Maximum Procode'+proCode.allowedPromote,
            };
        ...
        console.log('-09-0-0-0-908097896785674563453543hgjugkjhlgfuiludliuil    :'+p.length+' > '+proCode.allowedPromote);
    }
        ...
        updateProduct.proCode = product.proCode;
        updateProduct.vendor = product.vendor;
