...
function authorizationChecker(connection) {
    ...
    return function innerAuthorizationChecker(action, roles) {
        return tslib_1.__awaiter(this, void 0, void 0, function* () {
            ...
            const user = yield authService.parseBasicAuthFromRequest(action.request);
            if (user === undefined) {
            ...
            console.log(roles);
            if (roles[0] === 'customer') {
                action.request.user = yield authService.validateCustomer(user.id);
                ...
                if (user.usertype == 'admin') {
                    action.request.user = yield authService.validateUser(user.id);
                    if (action.request.user === undefined) {
                        log.warn('Invalid credentials given');
                        return false;
                    }
                    ...
                    action.request.user = yield authService.validateUserVendor(user.id);
                    if (action.request.user === undefined) {
                        log.warn('Invalid credentials given');
                        return false;
                    }
