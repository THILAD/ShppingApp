...
let ProductService = class ProductService {
    ...
    list_vendor(limit, offset, select = [], relation = [], whereConditions = [], search = [], price, count, vendor = 0) {
        const condition = {};
        if (select && select.length > 0) {
            condition.select = select;
        }
    ...
    list_item_minimum_vendor(limit, offset, select = [], relation = [], whereConditions = [], search = [], price, count, vendor = 0, minimum = 0) {
        const condition = {};
        if (select && select.length > 0) {
            condition.select = select;
        }
        ...
        condition.where = "quantity < minimum_quantity";
        if (whereConditions && whereConditions.length > 0) {
            whereConditions.forEach((item) => {
                const operator = item.op;
                if (operator === 'where' && item.value !== '') {
    ...
    // Recent selling product
    recentProductSelling_vendor(limit, vendor) {
        return tslib_1.__awaiter(this, void 0, void 0, function* () {
            return yield this.productRepository.recentProductSelling_vendor(limit, vendor);
        });
    }
